...
export default function HashIndexInterface() {
    ...
    const handleTableScan = async () => {
        ...
        try {
            const response = await fetch(`${API_BASE}/search_scan/${encodeURIComponent(searchKey)}?max_records=50`)
            if (response.ok) {
                const data = await response.json()
                setScanResult(data)
                setScanRecords(data.scanned_records || [])
                setScanProgress(100)
            } else {
                const errorData = await response.json()
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div className="flex items-end">
                                <Button onClick={handleLoadData} disabled={loading || tableData !== null}
                                        className="w-full">
                ...
                {tableData && (
                    <Tabs defaultValue="visualization" className="space-y-6">
                        ...
                        <TabsContent value="visualization" className="space-y-6">
                            <HashVisualization tableData={tableData} statistics={statistics}
                        ...
                        <TabsContent value="search" className="space-y-6">
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Digite uma palavra para buscar usando o índice
                                        hash</CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    <div className="flex gap-2">
                                        ...
                                        />
                                        <Button onClick={handleHashSearch}
                                                disabled={!isIndexBuilt || loading || !searchKey.trim()}>
                                            Buscar
                                        </Button>
                                    </div>

                                    {searchResult &&
                                        <SearchResults result={searchResult} type="hash" searchKey={searchKey}/>}
                                </CardContent>
                            </Card>
                        </TabsContent>
                                    ...
                                    {loading && (
                                        <div className="space-y-2">
                                            ...
                                            <Progress value={undefined}
                                                      className="w-full"/> {/* Indicador de loading indeterminado */}
                                        </div>
                                    )}

                                    <TableScanVisualization
                                        records={scanRecords}
                                        isScanning={loading}
                                        totalScanned={scanResult?.total_scanned}
                                        limitedView={scanResult?.limited_view}
                                    />

                                    {scanResult &&
                                        <SearchResults result={scanResult} type="scan" searchKey={searchKey}/>}
                                </CardContent>
                            </Card>
                        </TabsContent>
